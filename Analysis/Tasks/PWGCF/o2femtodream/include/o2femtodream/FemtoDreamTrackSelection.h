...
class FemtoDreamTrackSelection : public FemtoDreamObjectSelection<float, femtoDreamTrackSelection::TrackSel>
{
 public:
  ...
  template <typename T>
  unsigned long int getCutContainer(T const& track);

  template <typename T>
  void fillQA(T const& track);

  template <typename T>
  void fillCutQA(T const& track, unsigned long int cutContainer);
...
void FemtoDreamTrackSelection::init(HistogramRegistry* registry)
{
  if (registry) {
    ...
    if (8 * sizeof(unsigned long int) < nSelections) {
    ...
    const int nChargeSel = getNSelections(femtoDreamTrackSelection::kCharge);
...
template <typename T>
inline auto FemtoDreamTrackSelection::getNsigmaTPC(T const& track, o2::track::PID pid)
{
  switch (pid) {
...
template <typename T>
inline auto FemtoDreamTrackSelection::getNsigmaTOF(T const& track, o2::track::PID pid)
{
...
template <typename T>
inline unsigned long int FemtoDreamTrackSelection::getCutContainer(T const& track)
{
  ...
  const static int nEtaMaxSel = getNSelections(femtoDreamTrackSelection::kEtaMax);
  ...
  const static float sClsMax = getMinimalSelection(femtoDreamTrackSelection::kTPCsClsMax, femtoDreamSelection::kLowerLimit);
  ...
  if (nEtaMaxSel > 0 && std::abs(eta) > etaMax) {
    return 0;
  }
  ...
  if (nTPCsMaxSel > 0 && tpcNClsS > sClsMax) {
    return 0;
  }
  ...
  if (nPIDnSigmaSel > 0) {
    if (std::all_of(pidTPC.begin(), pidTPC.end(), [](float pid) { return std::abs(pid) > nSigmaPIDMax; }) && std::all_of(pidTOF.begin(), pidTOF.end(), [](float pid) { return std::abs(pid) < nSigmaPIDMax; })) {
      return 0;
    }
  }

  // now iterate over all selections and check whether these are fulfilled
  float observable;
  for (auto& sel : mSelections) {
    ...
    if (selVariable == femtoDreamTrackSelection::kPIDnSigmaMax) {
      ...
      switch (selVariable) {
        case (femtoDreamTrackSelection::kCharge):
          observable = sign;
          break;
...
template <typename T>
inline void FemtoDreamTrackSelection::fillCutQA(T const& track, unsigned long int cutContainer)
{
  if (mHistogramRegistry) {
    size_t counter = 2; // first two slots reserved for track/v0/cascade encoding
    const auto sign = track.sign();
    ...
    femtoDreamTrackSelection::TrackSel oldTrackSel = femtoDreamTrackSelection::kCharge;
