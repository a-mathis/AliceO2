...
class FemtoDreamContainer
{
 ...
 public:
  ...
  template <typename T>
  void setPair(T const& part1, T const& part2);
...
void FemtoDreamContainer::init(o2::framework::HistogramRegistry* registry, Observable obs)
{
  ...
  mHistogramRegistry->add("relPairDist", ("; " + femtoObs + "; Entries").c_str(), o2::framework::kTH1F, {{5000, 0, 5}});
  mHistogramRegistry->add("relPairkT", "; #it{k}_{T} (GeV/#it{c}); Entries", o2::framework::kTH1F, {{5000, 0, 5}});
}

template <typename T>
inline void FemtoDreamContainer::setPair(T const& part1, T const& part2)
{
  float femtoObs;
  if (mFemtoObs == Observable::kstar) {
    femtoObs = getkstar(part1, mMassOne, part2, mMassTwo);
  }
  const float kT = getkT(part1, mMassOne, part2, mMassTwo);
