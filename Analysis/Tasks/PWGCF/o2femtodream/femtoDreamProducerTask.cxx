...
#include "include/o2femtodream/FemtoDreamCollisionSelection.h"
#include "include/o2femtodream/FemtoDreamTrackSelection.h"
...
struct femtoDreamProducerTask {
  ...
  Configurable<std::vector<float>> ConfTrkPtmin{"ConfTrkPtmin", std::vector<float>{0.4f, 0.6f, 0.5f, 0.2f}, "Trk sel: Min. pT (GeV/c)"};
  ...
  Configurable<std::vector<float>> ConfTrkTPCnclsMin{"ConfTrkTPCnclsMin", std::vector<float>{80.f, 70.f, 90.f}, "Trk sel: Min. nCls TPC"};
  ...
  Configurable<std::vector<float>> ConfTrkDCAxyMax{"ConfTrkDCAxyMax", std::vector<float>{0.1f, 999.f}, "Trk sel: Max. DCA_xy (cm)"}; /// here we need an open cut to do the DCA fits later on!
  Configurable<std::vector<float>> ConfTrkDCAzMax{"ConfTrkDCAzMax", std::vector<float>{0.2f, 999.f}, "Trk sel: Max. DCA_z (cm)"};    /// open cut needed for v0s etc. also there we need a lower limit bit
  ...
  /// todo with configurables - how to make sure we use the smallest/largest value?
  Filter trackFilter = aod::track::pt > 0.1f && nabs(aod::track::eta) < 0.9f;
  ...
  void init(InitContext&)
  {
    ...
    trackCuts.setSelection(ConfTrkCharge, femtoDreamTrackSelection::kCharge, femtoDreamSelection::kEqual);
