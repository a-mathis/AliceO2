...
class FemtoDreamParticleHisto
{
 public:
  ...
  void fillQA(T const& part)
  {
    if (mHistogramRegistry) {
      ...
      if constexpr (mParticleType == o2::aod::femtodreamparticle::ParticleType::kTrack) {
        mHistogramRegistry->fill(HIST(o2::aod::femtodreamparticle::ParticleTypeName[mParticleType]) + HIST(mFolderSuffix[mFolderSuffixType]) + HIST("/dcaXYhist"), part.pt(), part.tempFitVar());
      } else if constexpr (mParticleType == o2::aod::femtodreamparticle::ParticleType::kV0) {
        mHistogramRegistry->fill(HIST(o2::aod::femtodreamparticle::ParticleTypeName[mParticleType]) + HIST(mFolderSuffix[mFolderSuffixType]) + HIST("/cpahist"), part.pt(), part.tempFitVar());
      }
    }
  }
 ...
 private:
  ...
  static constexpr int mFolderSuffixType = suffixType;
  static constexpr std::string_view mFolderSuffix[3] = {"", "_one", "_two"};
};
} // namespace o2::analysis::femtoDream
