...
bool FemtoDreamTrackSelection::isSelectedMinimal(T const& track)
{
  ...
  if (nDCAMinSel > 0 && std::abs(dcaXY) < dcaMin && std::abs(dcaZ) < dcaMin) {
    return false;
  }
  return true;
}
...
void FemtoDreamTrackSelection::fillQA(T const& track)
{
  if (mHistogramRegistry) {
    ...
    mHistogramRegistry->fill(HIST("TrackCuts/dcahist"), track.dcaXY());
    ...
    for (auto& sel : mSelections) {
      ...
      if (selVariable == femtoDreamTrackSelection::kPIDnSigmaMax) {
        ...
        switch (selVariable) {
          ...
          case (femtoDreamTrackSelection::kDCAMin):
            if (isTrue) {
              mHistogramRegistry->fill(HIST("TrackCutsQA/dcaMin"), currentTrackSelCounter, dcaXY);
            }
            break;
          case (femtoDreamTrackSelection::kPIDnSigmaMax):
            break;
